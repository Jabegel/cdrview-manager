...
export default function Configuracoes(){
  ...
  async function load(){
    const r = await fetch('/api/configs/list'); const j = await r.json(); setConfigs(j.configs||[])
  }
  ...
  async function remove(id){ if(!confirm('Excluir?')) return; await fetch('/api/configs/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id})}); await load() }
  ...
  const filtered = configs.filter(c => { if(!filter) return true; const q=filter.toLowerCase(); return (c.nome||'').toLowerCase().includes(q) || (c.exe||'').toLowerCase().includes(q) })
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h2>Configurações</h2>
        <div>
          <input className="form-control d-inline-block me-2" style={{width:220}} placeholder="Buscar..." value={filter} onChange={e=>setFilter(e.target.value)} />
      ...
      {showForm && <div className="card"><div className="card-body">
        ...
        <form onSubmit={save}>
          ...
          <div className="d-flex gap-2"><button className="btn btn-primary" type="submit">Salvar</button><button className="btn btn-secondary" type="button" onClick={()=>setShowForm(false)}>Cancelar</button></div>
        </form>
      </div></div>}
